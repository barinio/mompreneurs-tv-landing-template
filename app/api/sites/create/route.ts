...
import { forkRepo, writeContentJson, readSitesJson, writeSitesJson } from '@/lib/github'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 2. Wait briefly for fork to be ready, then reset content.json to blank template
    await new Promise((r) => setTimeout(r, 3000))
    await writeContentJson(owner, name, contentDefault, '')

    // 3. Create Vercel project linked to forked repo
    const project = await createVercelProject(name, owner, name, githubToken)
    ...
    const siteUrl = await triggerDeploy(project.id)
